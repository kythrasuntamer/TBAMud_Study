...
/* iedit MVP test-link stubs.
 * The real game links these from src/oedit.c, but test_interpreter
 * only links interpreter.c + test_stubs.c.
 */
...
void do_oasis_iedit(struct char_data *ch, char *argument, int cmd, int subcmd)
{
  (void)ch;
  (void)argument;
  (void)cmd;
